...
level_name = input()
try:
    f = open("levels/platforms/" + level_name + ".json")
    data = json.load(f)
    f.close()
except FileNotFoundError:
    f = open("levels/platforms/" + level_name + ".json", "a")
    f.close()
    data = {}
    data["platforms"] = []

f = open("levels/platforms/" + level_name + ".json", "w")
...
print('write stop to stop editing and save changes')
try:
    f = open("levels/coins/" + level_name + ".json")
    data = json.load(f)
    f.close()
except FileNotFoundError:
    f = open("levels/coins/" + level_name + ".json", "a")
    f.close()
    data = {}
    data["platforms"] = []
f = open("levels/coins/" + level_name + ".json", "w")
k = input()
while k != "stop":
    x, y, width, height = k.split()
    x = int(x)
...
background = input()
f = open("levels/background/" + level_name + ".txt", "w")
